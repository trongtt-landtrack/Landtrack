...
const CUNG_8 = ["Khảm", "Cấn", "Chấn", "Tốn", "Ly", "Khôn", "Đoài", "Càn"];
...
export default function FengShuiPage() {
  ...
  const getDirectionName = (h: number) => {
    const dir = BAGUA_DIRECTIONS.find((d) => {
      if (d.range2)
        return (
          (h >= d.range[0] && h <= d.range[1]) ||
          (h >= d.range2[0] && h <= d.range2[1])
        );
  ...
  const getDirectionNameDetailed = (h: number) => {
    const sonIndex = Math.floor(((h + 7.5) % 360) / 15);
    const son = SON_24[sonIndex];
    const batQuai = getDirectionName(h);
    return `${son} (${batQuai})`;
  };
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 mb-24">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 flex flex-col items-center justify-center min-h-[400px]">
            {!browserSupported ? (
              <div className="text-center p-6 bg-red-50 text-red-600 rounded-2xl border border-red-100">
                ...
                <p>
                  Thiết bị hoặc trình duyệt của bạn không hỗ trợ cảm biến hướng.
                </p>
              </div>
              ...
              <div className="text-center p-8 bg-slate-50 rounded-2xl w-full border border-slate-100">
                ...
                <p className="text-slate-600 mb-6 font-sans text-sm block">
                  Cấp quyền cảm biến để xem tọa độ thực tế tại dự án. Để điện
                  thoại nằm ngang hình số 8.
                </p>
                <button
              ...
              <div className="flex flex-col items-center w-full relative">
                {/* Heading Labels */}
                <div className="bg-primary/5 border border-primary/20 px-8 py-2 rounded-full mb-3 shadow-sm">
                  <p className="text-3xl font-display font-black text-primary tracking-tighter tabular-nums drop-shadow-sm">
                    {heading !== null ? `${heading}°` : "--°"}
                  </p>
                </div>
                <div className="bg-gradient-gold border border-accent/30 px-8 py-2 rounded-full mb-10 shadow-md text-center max-w-[280px] w-full">
                  <p className="text-sm font-display font-bold text-white uppercase tracking-widest">
                    HƯỚNG:{" "}
                    {heading !== null
                      ? getDirectionNameDetailed(heading).toUpperCase()
                      : "---"}
                  </p>
                </div>

                <div className="relative w-[340px] h-[340px] flex items-center justify-center">
                  {/* Fixed Target Marker (12 o'clock) */}
                  <div className="absolute top-[-10px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[12px] border-r-[12px] border-t-[16px] border-l-transparent border-r-transparent border-t-red-600 z-50 shadow-md"></div>
                  <div className="absolute top-[-14px] left-1/2 -translate-x-1/2 w-1 h-[40px] bg-red-600/50 z-40"></div>

                  <motion.div
                    className="relative w-full h-full rounded-full shadow-2xl flex items-center justify-center bg-white border-2 border-slate-200 overflow-hidden"
                    animate={{ rotate: heading !== null ? -heading : 0 }}
                    transition={{ type: "tween", duration: 0.1 }}
                  >
                    {/* R1: Degrees (0-360) */}
                    <div className="absolute inset-0 rounded-full bg-slate-50 border-[2px] border-slate-200">
                      {Array.from({ length: 72 }).map((_, i) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-[1px] bg-slate-400 ${isTen ? "h-[6px]" : "h-[3px]"}`}
                            ></div>
                            {isTen && (
                              <div className="absolute top-2.5 text-[8px] font-mono text-slate-500 font-bold">
                                {deg}°
                              </div>
                            )}
                          </div>
                    ...
                    {/* R2: 24 Sơn Hướng */}
                    <div className="absolute inset-[30px] rounded-full border border-slate-300 bg-white shadow-inner">
                      {Array.from({ length: 24 }).map((_, i) => (
                        <div
                          key={`son-div-${i}`}
                          className="absolute inset-0 flex items-start justify-center"
                          style={{ transform: `rotate(${i * 15 + 7.5}deg)` }}
                        >
                          <div className="w-[1px] h-full bg-slate-200 mb-1/2"></div>
                        </div>
                      ))}
                      {SON_24.map((son, i) => {
                        const isPrimary = ["Tý", "Ngọ", "Mão", "Dậu"].includes(
                          son,
                        );
                        const isAccent = ["Càn", "Khôn", "Cấn", "Tốn"].includes(
                          son,
                        ); // 4 góc
                        return (
                          <div
                          ...
                          >
                            <span
                              className={`text-[9px] font-bold ${isPrimary ? "text-red-600" : isAccent ? "text-accent" : "text-slate-600"}`}
                            >
                              {son}
                            </span>
                          </div>
                    ...
                    {/* R3: 8 Bát Quái */}
                    <div className="absolute inset-[65px] rounded-full border border-slate-300 bg-slate-50 shadow-inner">
                    ...
                    {/* R4: 8 Cung */}
                    <div className="absolute inset-[115px] rounded-full border border-slate-200 bg-gradient-gold shadow-[inset_0_2px_10px_rgba(0,0,0,0.2)]">
                      {Array.from({ length: 8 }).map((_, i) => (
                        <div
                          key={`cung-div-${i}`}
                          className="absolute inset-0 flex items-start justify-center"
                          style={{ transform: `rotate(${i * 45 + 22.5}deg)` }}
                        >
                          <div className="w-[1px] h-full bg-accent/30"></div>
                        </div>
                      ))}
                      {CUNG_8.map((cung, i) => (
                        <div
                        ...
                        >
                          <span className="text-[11px] font-black text-white uppercase drop-shadow-sm">
                            {cung}
                          </span>
                        </div>
                      ))}
                    ...
                    {/* Center Knob */}
                    <div className="absolute w-[60px] h-[60px] rounded-full bg-gradient-to-br from-[#1e293b] to-[#0f172a] shadow-[0_5px_15px_rgba(0,0,0,0.5)] border-4 border-white flex items-center justify-center z-10">
                      <div className="w-1.5 h-1.5 rounded-full bg-red-500 absolute top-1.5"></div>
                      <Compass className="w-6 h-6 text-accent" />
                    </div>

                    {/* Crosshair guidelines on plate */}
                ...
                {/* Toa Labels (Opposite Direction) */}
                <div className="mt-12 bg-white rounded-3xl p-6 shadow-xl border border-slate-100 max-w-[340px] w-full text-center relative overflow-hidden">
                  <div className="absolute top-0 inset-x-0 h-1 bg-gradient-gold"></div>
                  <div className="bg-gradient-gold border border-accent/30 px-8 py-2 rounded-full mb-4 shadow-md mx-auto max-w-fit mt-2">
                    <p className="text-sm font-display font-bold text-white uppercase tracking-widest">
                      TỌA:{" "}
                      {heading !== null
                        ? getDirectionNameDetailed(
                            (heading + 180) % 360,
                          ).toUpperCase()
                        : "---"}
                    </p>
                  </div>
                  <div className="bg-slate-100 border border-slate-200 px-6 py-1.5 rounded-full mx-auto max-w-fit shadow-inner mb-6">
                    <p className="text-xl font-display font-black text-slate-700 tracking-tighter tabular-nums drop-shadow-sm">
                      {heading !== null ? `${(heading + 180) % 360}°` : "--°"}
                    </p>
                  </div>

                  <div className="flex items-center justify-center gap-12">
                    <div className="flex flex-col items-center gap-2">
                      <button className="w-14 h-14 rounded-full bg-gradient-to-br from-[#1e293b] to-[#0f172a] shadow-[0_5px_15px_rgba(0,0,0,0.3)] border-2 border-accent/50 flex items-center justify-center text-accent hover:scale-105 transition-transform">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 flex flex-col justify-center">
            ...
            <div className="space-y-4 text-sm font-sans text-slate-600 leading-relaxed bg-slate-50 p-6 rounded-2xl">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li>
                  Đứng cách xa các vật kim loại lớn, trụ điện, loa hoặc thiết bị
                  từ tính.
                </li>
                <li>
                  Mở cửa chính, đứng từ tâm nhà hoặc tâm lô đất nhìn thẳng ra
                  ngoài.
                </li>
                <li>
                  Kết quả từ la bàn điện thoại mang tính chất tham khảo, có thể
                  sai số 5-10 độ so với la bàn chuyên dụng (La Kinh).
                </li>
              </ul>
            </div>
            ...
            {result && (
              <div className="mt-6 p-6 border-2 border-accent/20 bg-accent/5 rounded-2xl text-center">
                <p className="text-sm text-slate-600 mb-2 font-display uppercase tracking-widest font-bold">
                  Gợi ý cho CĐT sinh năm {birthYear}
                </p>
                <p className="font-sans text-sm text-slate-700">
                  ...
                  <br />
                  để chỉ cho khách hàng vị trí đón Sinh Khí.
                </p>
              </div>
            )}
          </div>
      ...
      {activeTab === "tuvan" && (
        <div className="animate-in fade-in slide-in-from-bottom-4 w-full space-y-8">
          {/* Form */}
          <div
            className={`transition-all duration-500 ${!result ? "max-w-3xl mx-auto mt-12" : "w-full"}`}
          >
            <div className="bg-white rounded-[2rem] p-8 md:p-10 shadow-xl border border-gray-100 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="text-center mb-8">
                  <h2 className="text-2xl md:text-3xl font-display font-black text-primary uppercase tracking-widest mb-2">
                    Hồ Sơ Cung Mệnh
                  </h2>
                  <p className="text-sm font-sans text-gray-500">
                    Tra cứu bát trạch, hướng nhà và ngày giờ tốt theo tuổi
                  </p>
                </div>

                <div
                  className={`flex flex-col ${result ? "md:flex-row md:items-end" : ""} gap-6`}
                >
                  <div className="flex-1">
                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">
                      Năm sinh Âm lịch (Của Gia Chủ)
                    </label>
                    <input
                      type="number"
                  ...
                  <div className="flex-1">
                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3 text-center md:text-left">
                      Giới tính
                    </label>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8">
              <div className="bg-white rounded-[2rem] p-8 md:p-10 shadow-xl border border-gray-100">
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-gray-100">
                  ...
                  <div>
                    <h2 className="text-xl font-display font-black text-gray-800 uppercase tracking-widest">
                      Phân tích Bát Trạch
                    </h2>
                    <p className="text-sm font-sans text-gray-500">
                      Kết quả phi cung mệnh theo năm sinh {birthYear}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 xl:gap-8 mb-10">
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6 rounded-3xl border border-gray-200 text-center lg:text-left relative overflow-hidden group hover:border-gray-300 transition-colors">
                    <p className="text-xs sm:text-sm font-black text-gray-500 uppercase tracking-widest mb-2">
                      Can Chi
                    </p>
                    <p className="text-xl sm:text-2xl xl:text-3xl font-display font-black text-primary truncate leading-tight py-1">
                      {result.canChi}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-4 sm:p-6 rounded-3xl border border-accent/20 text-center lg:text-left relative overflow-hidden group hover:border-accent/40 transition-colors">
                    ...
                    <p className="text-xs sm:text-sm font-black text-accent/80 uppercase tracking-widest mb-2">
                      Ngũ Hành
                    </p>
                    <p className="text-xl sm:text-2xl xl:text-3xl font-display font-black text-accent truncate leading-tight py-1">
                      Mệnh {result.mang}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-4 sm:p-6 rounded-3xl border border-blue-100 text-center lg:text-left hover:border-blue-200 transition-colors">
                    <p className="text-xs sm:text-sm font-black text-blue-700/60 uppercase tracking-widest mb-2">
                      Cung Phi
                    </p>
                    <p className="text-xl sm:text-2xl xl:text-3xl font-display font-black text-blue-700 truncate leading-tight py-1">
                      Cung {result.cung}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-emerald-50 to-teal-50 p-4 sm:p-6 rounded-3xl border border-emerald-100 text-center lg:text-left hover:border-emerald-200 transition-colors">
                    <p className="text-xs sm:text-sm font-black text-emerald-700/60 uppercase tracking-widest mb-2">
                      Thiên Hướng
                    </p>
                    <p className="text-lg sm:text-xl xl:text-2xl font-display font-black text-emerald-700 truncate leading-tight py-1 mt-1">
                      {result.trach}
                    </p>
                  </div>
                </div>
                    ...
                    <ul className="space-y-4 relative z-10">
                      {result.huongTot.map((h: string, idx: number) => {
                        ...
                        return (
                          <li
                            key={idx}
                            className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-0 group"
                          >
                            <span className="font-bold text-gray-700 text-lg group-hover:text-emerald-700 transition-colors">
                              {dir}
                            </span>
                            <span className="text-xs font-black text-emerald-700 bg-emerald-100 px-4 py-2 rounded-xl border border-emerald-200/50 self-start sm:self-auto text-center">
                              {meaning.replace(")", "")}
                            </span>
                          </li>
                        );
                      ...
                      {result.huongXau.map((h: string, idx: number) => {
                        ...
                        return (
                          <li
                            ...
                            className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-0 group"
                          >
                            <span className="font-bold text-gray-700 text-lg group-hover:text-red-700 transition-colors">
                              {dir}
                            </span>
                            <span className="text-xs font-black text-red-700 bg-red-100 px-4 py-2 rounded-xl border border-red-200/50 self-start sm:self-auto text-center">
                              {meaning.replace(")", "")}
                            </span>
                          </li>
                        );
              ...
              <div className="bg-white rounded-[2rem] p-8 md:p-10 shadow-xl border border-gray-100 relative overflow-hidden">
                <div className="flex items-center gap-4 mb-8 pb-6 border-b border-gray-100 relative z-10">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-sans text-gray-500">
                      Các ngày tốt trong tháng hợp với Mệnh {result.mang} (
                      {result.canChi})
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 xl:gap-8 relative z-10">
                  {goodDays.map((day, idx) => (
                    <div
                      key={idx}
                      className="bg-slate-50 border border-gray-100 p-6 md:p-8 rounded-[2rem] hover:border-accent/40 transition-all hover:shadow-lg group relative overflow-hidden"
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6 relative z-10">
                        <div>
                          ...
                          <p className="text-xl sm:text-2xl xl:text-3xl font-display font-black text-primary group-hover:text-accent transition-colors">
                            {day.title}
                          </p>
                        </div>
                      ...
                      <div className="flex items-start gap-3 mt-4 relative z-10">
                        ...
                        <p className="text-sm text-gray-500 pt-1.5 leading-relaxed">
                          {day.desc}
                        </p>
                      </div>
                    </div>
